...
// 帮助按钮
var helpButton = buttonRow3.add("button", undefined, "?");
...
function showHelpWindow() {
    ...
    descText = descPanel.add("statictext", undefined, "• Alt + 点击：直接替换原图", undefined);
    ...
    usageText = usagePanel.add("statictext", undefined, "3. 点击\"开始压缩\"或使用快捷键组合", undefined);
...
function compressImage(apiKey, inputFile, outputFile, keyIndex) {
    ...
    try {
        ...
        return { success: success, size: outputFileObj.exists ? outputFileObj.length : 0 };
...
uploadButton.onClick = function() {
    ...
    if (ctrlPressed && shiftPressed) {
        ...
        // 询问是否替换原图
        var replaceResult = confirm("找到 " + selectedFiles.length + " 个选中的图片文件\n\n是否直接替换原图？\n\n是 = 替换原图\n否 = 压缩到临时文件夹");
        
        var replaceOriginal = (replaceResult === true);
        ...
        addLog("开始压缩图片..." + (replaceOriginal ? "（将直接替换原图）" : "（压缩到临时文件夹）"));
        statusText.text = "状态：开始压缩...";
        
        var result = compressSelectedFiles(selectedFiles, replaceOriginal);
...
replaceButton.onClick = function() {
    ...
    var tempOutputFolder = new Folder(Folder.temp.fsName + "/auto_tinify_output");
