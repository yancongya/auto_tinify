...
var SCRIPT_SUBTITLE = "主打简单快捷的图片压缩工具（支持AE/PS）";
...
var LINK_BUTTON_2_URL = "https://yancongya.github.io/auto_tinify/";
...
function openPathSettingsWindow() {
    ...
    helpButton.onClick = function() {
        var promptText = "给 AI 的 Prompt：\n\n" +
            "我需要为图片压缩配置路径，请帮我生成以下情况的路径模式：\n\n" +
            "【可用变量】\n" +
            "${projectPath} - 当前项目/文档文件所在的父目录\n\n" +
            "【常见场景示例】\n" +
...
uploadButton.onClick = function() {
    ...
    if (!(sourceFolder instanceof Folder)) {
        if (isPhotoshop) {
            alert("文档未保存，请先保存文档！\n\nPhotoshop 需要保存文档后才能使用 ${projectPath} 变量。");
            addLog("错误：文档未保存！");
        } else {
            alert("项目未保存，请先保存项目！");
            addLog("错误：项目未保存！");
        }
        statusText.text = "状态：错误，文档未保存";
        return;
    }
...
addLog("运行环境: " + (isPhotoshop ? "Adobe Photoshop" : "After Effects"));
