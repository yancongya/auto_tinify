...
function copyFolder(source, destination) {
    ...
    var copiedCount = 0; // 记录复制的文件数量
    for (var i = 0; i < files.length; i++) {
        var file = files[i];
        var destFile = new File(destination.fsName + "/" + file.name);
        if (file instanceof Folder) {
            copiedCount += copyFolder(file, destFile); // 递归复制子文件夹，将返回值累加
        } else {
            // 检查文件扩展名，只复制图片文件
    ...
    if (sourceFolder.exists) {
        ...
        if (tinyExePath.exists) {
            ...
            if ($.os.indexOf("Windows") === -1) {
                ...
                // 定义工作目录（解决默认路径问题）
                var workingDirectory = "D:\\auto_tiny"; // 替换为实际有效路径
                ...
                if (!file.exists) {
                    ...
                    try {
                        ...
                        var command = 'cmd /c "cd /d ' + workingDirectory + ' && tiny.exe"';
...
replaceButton.onClick = function() {
    ...
    if (outputFolder.exists) {
        ...
        // 将输出文件夹中的文件复制到目标路径（替换文件）
        copyFolder(outputFolder, destinationFolder);
